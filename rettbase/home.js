...
window.addEventListener("DOMContentLoaded", () => {
  ...
  function waitForAuthData() {
    return new Promise((resolve) => {
      // ⚡ PWA-FIX: Längeres Timeout für WebApps (10 Sekunden statt 5)
      // Beim ersten Laden einer WebApp benötigt das Dashboard mehr Zeit
      const AUTH_TIMEOUT = 10000; // 10 Sekunden Timeout für WebApps
      let messageHandler = null;
      let timeoutId = null;
      
      // 1. Sende "Ich bin bereit" Nachricht an das Parent-Fenster
      if (window.parent && window.parent !== window) {
         // Sende mehrfach, falls die erste Nachricht verpasst wird
         const sendReady = () => {
         ...
         // Auch nach kurzer Verzögerung senden (für PWA-Modus)
      ...
      messageHandler = (event) => {
        // ⚡ WICHTIG: Akzeptiere Nachrichten vom Parent, auch wenn event.source !== window.parent
        // Im PWA-Modus kann event.source unterschiedlich sein
      ...
      // 3. Timeout-Fallback: Falls keine AUTH_DATA Nachricht kommt
      timeoutId = setTimeout(() => {
        console.warn("⚠️ Timeout: Keine AUTH_DATA Nachricht nach", AUTH_TIMEOUT / 1000, "Sekunden empfangen. Verwende Fallback.");
        window.removeEventListener('message', messageHandler);
        // Versuche Firebase Auth direkt zu verwenden
        if (auth && auth.currentUser) {
          console.log("🔄 Versuche Firebase Auth direkt zu verwenden...");
          ...
          console.warn("⚠️ Keine Firebase Auth verfügbar - verwende Gast-Zugriff");
          resolve({ role: 'user', companyId: 'guest', uid: 'unknown' });
        }
      }, AUTH_TIMEOUT);
    });
  ...
  async function mergeFromFirestore() {
    ...
    try {
      // Firestore: kunden/{companyId}/users/{uid}/userTiles/config
      const tilesRef = doc(db, "kunden", userAuthData.companyId, "users", userAuthData.uid, "userTiles", "config");
      const tilesSnap = await getDoc(tilesRef);
      
      if (!tilesSnap.exists()) {
        // Dokument existiert nicht - erstelle es mit leeren Kacheln
      ...
      const data = tilesSnap.data();
      let remote = data.tiles;
      
      // Parse tiles aus JSON-String (falls als String gespeichert - Kompatibilität)
      if (typeof remote === 'string') {
        try {
          remote = JSON.parse(remote);
        } catch (e) {
          console.warn("⚠️ Konnte tiles nicht parsen:", e);
          remote = null;
        }
      }

      if (!remote || !Array.isArray(remote)) {
        // Keine Remote-Daten: Verwende leere Kacheln-Array
        const emptyTiles = Array(totalTiles).fill(null);
        await setDoc(tilesRef, {
          tiles: emptyTiles,
          updatedAt: serverTimestamp()
  ...
  async function saveToFirestore() {
    if (!userAuthData || !userAuthData.companyId || !userAuthData.uid) return;
    ...
    try {
      ...
      console.log("✅ Kacheln in Firestore gespeichert");
    } catch (e) {
  ...
  async function checkUnreadEmails() {
    ...
    try {
      // Firestore: kunden/{companyId}/emails
      ...
      // Collection existiert möglicherweise noch nicht - ignoriere Fehler
  ...
  /**
   * Lädt den Vorname des Benutzers
   * ⚡ OPTIMIERT: Verwendet bereits geladene Mitarbeiter-Daten vom Dashboard, falls vorhanden
   */
  async function loadUserVorname() {
    ...
    // ⚡ OPTIMIERT: Verwende bereits geladene Mitarbeiter-Daten vom Dashboard
    if (userAuthData.mitarbeiterData && userAuthData.mitarbeiterData.vorname) {
      return userAuthData.mitarbeiterData.vorname.trim();
    }
    ...
    try {
      ...
      if (mitarbeiterSnap.exists()) {
        const data = mitarbeiterSnap.data();
      ...
      if (!uidSnapshot.empty) {
        ...
        const data = mitarbeiterDoc.data();
        const vorname = data.vorname || null;
        if (vorname) {
          return vorname.trim();
        }
  ...
  /**
   * Formatiert den Wochentag als 2-stellige Abkürzung mit Punkt (für Mobile)
   */
  ...
  /**
   * Prüft, ob die aktuelle Ansicht mobile ist (Bildschirmbreite <= 480px)
   */
  ...
  /**
   * Aktualisiert die angezeigte Uhrzeit und das Datum
   * Desktop: "Tag, TT. Monat HH:MM" (z.B. "Montag, 15. Dezember - 14:30")
   * Mobile: "Tag TT.MM HH:MM" (z.B. "Sa. 15.12 - 14:30")
   */
  function updateTime() {
    ...
    // Prüfe ob Mobile-Ansicht
    if (isMobileView()) {
      // Mobile: "Tag TT.MM HH:MM" (z.B. "Sa. 15.12 - 14:30")
      const dayNameShort = getDayNameShort(now);
      const day = String(now.getDate()).padStart(2, '0');
      const month = String(now.getMonth() + 1).padStart(2, '0');
      const dateString = `${day}.${month}`;
      
      dateTimeElement.textContent = `${dayNameShort} ${dateString} - ${timeString}`;
    } else {
      // Desktop: "Tag, TT. Monat HH:MM" (z.B. "Montag, 15. Dezember - 14:30")
      ...
      dateTimeElement.textContent = `${dayName}, ${day}. ${month} - ${timeString}`;
    }
  }
  
  // Event: Re-render bei Resize für responsive Umschaltung
  if (!window.hasResizeListenerForHome) {
    window.hasResizeListenerForHome = true;
    let resizeTimeout;
    window.addEventListener('resize', () => {
      clearTimeout(resizeTimeout);
      resizeTimeout = setTimeout(() => {
        // Re-render Datum/Zeit wenn sich die Bildschirmgröße ändert
        updateTime();
      }, 100);
    });
  }
