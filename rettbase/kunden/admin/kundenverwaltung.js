 ...
 * @returns {Object} Objekt mit moduleId -> enabled Status
 */
function getSelectedModules(container) {
    ...
    checkboxes.forEach(checkbox => {
        ...
        if (moduleId) {
            modules[moduleId] = checkbox.checked;
        }
    });
    
    // Home und Admin sind immer aktiv (für Kunden-Admin)
    modules['home'] = true;
    modules['admin'] = true;
...
companyForm.addEventListener('submit', async (e) => {
    ...
    const companySubdomain = document.getElementById("companySubdomain").value.toLowerCase().trim();
    ...
    try {
        ...
        const selectedModules = getSelectedModules(moduleCheckboxes);
        // Stelle sicher, dass Home und Admin immer aktiviert sind
        selectedModules['home'] = true;
        selectedModules['admin'] = true;
...
async function loadAndRenderCompanyList() {
    ...
    try {
        ...
        // Speichere alle Kunden für Filter/Suche
        allCompanies = [];
        ...
        customerList.innerHTML = '<p style="color:red;">Fehler beim Laden der Kundenliste. Prüfen Sie die Berechtigungen.</p>';
        allCompanies = [];
    }
}
...
editCompanyForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        const subdomain = editSubdomain.value.toLowerCase().trim();
        ...
        // Aktualisiere Module-Freischaltungen
        const selectedModules = getSelectedModules(editModuleCheckboxes);
        await setCompanyModules(companyId, selectedModules);
