...
import { FieldValue } from "https://www.gstatic.com/firebasejs/11.0.1/firebase-firestore.js";
...
// Offene Dienste Popup
const openShiftsBtn = document.getElementById("openShiftsBtn");
const openShiftsPopupOverlay = document.getElementById("openShiftsPopupOverlay");
const openShiftsPopupForm = document.getElementById("openShiftsPopupForm");
const openShiftsTextarea = document.getElementById("openShiftsTextarea");
const openShiftsSchichtartSelect = document.getElementById("openShiftsSchichtartSelect");
...
function getMitarbeiterCollection() {
  const companyId = getCompanyId();
  // 🔥 NEUE ZENTRALE MITARBEITER-COLLECTION
  return collection(db, "kunden", companyId, "mitarbeiter");
}
...
// Hilfsfunktion: Formatiert den Namen korrekt (ohne "undefined")
function formatMitarbeiterName(vorname, nachname) {
  if (!nachname) return "Unbekannt";
  if (!vorname || vorname.trim() === "") return nachname.trim();
  return `${vorname.trim()} ${nachname.trim()}`;
}

// Hilfsfunktion: Formatiert den Namen im Format "Nachname, Vorname" (für Personal-Popup)
function formatMitarbeiterNameForSelect(vorname, nachname) {
  if (!nachname) return "Unbekannt";
  if (!vorname || vorname.trim() === "") return nachname.trim();
...
async function initializeApp() {
  ...
  // Einstellungs-Button für OVD ausblenden
  if (settingsBtn) {
    if (userAuthData?.role === 'ovd') {
      settingsBtn.style.display = 'none';
    }
    // Für andere Rollen bleibt der Button sichtbar (Standard-Verhalten)
  }

  // Offene Dienste-Button nur für bestimmte Rollen anzeigen
  if (openShiftsBtn) {
    const allowedRolesForOpenShifts = ["superadmin", "admin", "supervisor", "rettungsdienstleiter", "wachleitung", "ovd"];
    const userRole = userAuthData?.role?.toLowerCase();
    const isAllowed = userRole && allowedRolesForOpenShifts.includes(userRole);
    
    if (isAllowed) {
      openShiftsBtn.style.display = 'block';
    } else {
      openShiftsBtn.style.display = 'none';
    }
  }
...
async function loadMitarbeiter() {
  try {
    // 🔥 LADE AUS NEUER ZENTRALER MITARBEITER-COLLECTION
    const mitarbeiterCollection = getMitarbeiterCollection();
    const snap = await getDocs(query(mitarbeiterCollection, orderBy("nachname", "asc")));
    
    // Mappe Daten und stelle Kompatibilität sicher
    allMitarbeiter = snap.docs.map((d) => {
      const data = d.data();
      
      // Kompatibilität: Falls qualifikationen vorhanden, verwende das, sonst qualifikation
      // (Schichtplan unterstützt beide Varianten)
      if (data.qualifikationen && !data.qualifikation) {
        data.qualifikation = data.qualifikationen;
      }
      
      // Kompatibilität: Stelle sicher, dass beide Felder (telefon und telefonnummer) vorhanden sind
      // für bestehenden Code, der telefonnummer verwendet
      // Priorität: telefon (neue zentrale Collection) > telefonnummer (alte Collection)
      const telefonValue = data.telefon || data.telefonnummer || "";
      if (telefonValue) {
        data.telefon = telefonValue;
        data.telefonnummer = telefonValue; // Für Kompatibilität mit bestehendem Code
      } else {
        // Stelle sicher, dass beide Felder gesetzt sind (auch wenn leer)
        data.telefon = data.telefon || "";
        data.telefonnummer = data.telefonnummer || "";
      }
      
      return { id: d.id, ...data };
    }).filter(m => m.active !== false); // Nur aktive Mitarbeiter
    
    console.log("👥 Mitarbeiter geladen aus zentraler Collection:", allMitarbeiter.length);
  } catch (e) {
    console.error("❌ Fehler beim Laden der Mitarbeiter:", e);
    allMitarbeiter = []; // Bei Fehler leeres Array
  }
}
...
async function updateMitarbeiterInAllShifts(mitarbeiterId, updatedMitarbeiterData) {
  try {
    ...
    const updatedName = formatMitarbeiterName(updatedMitarbeiterData.vorname, updatedMitarbeiterData.nachname);
...
async function renderShiftsForStandortAndDay(standortId, dayId, container) {
  ...
  try {
    ...
    // Sortiere Schicht-Slots nach Schichtname: Zahlen (1-100) zuerst, dann alphabetisch (A-Z)
    // Dies entspricht der Sortierung der Schichten in den Einstellungen
    rows.sort((a, b) => {
      // Hole den Schichtnamen für beide Schichten
      const nameA = getShiftDisplayName(standortId, a.id, a.data) || "";
      const nameB = getShiftDisplayName(standortId, b.id, b.data) || "";
      
      const nameATrim = nameA.trim();
      const nameBTrim = nameB.trim();
      
      // Prüfe, ob die Namen Zahlen sind
      const numA = parseInt(nameATrim, 10);
      const numB = parseInt(nameBTrim, 10);
      const isNumA = !isNaN(numA) && numA >= 1 && numA <= 100 && String(numA) === nameATrim;
      const isNumB = !isNaN(numB) && numB >= 1 && numB <= 100 && String(numB) === nameBTrim;
      
      // Zahlen kommen vor Text
      if (isNumA && isNumB) {
        return numA - numB; // Numerische Sortierung: 1, 2, 3, ..., 100
      }
      if (isNumA) return -1; // Zahl kommt vor Text
      if (isNumB) return 1;  // Text kommt nach Zahl
      
      // Beide sind Text: alphabetische Sortierung
      return nameATrim.localeCompare(nameBTrim, 'de', { sensitivity: 'base' });
    });
...
function renderShiftRowElement(dayId, standortId, shiftId, data) {
  ...
  if (shiftDefsForStandort.length === 0) {
    ...
    // Sortiere Schichten nach Name: Zahlen (1-100) zuerst, dann alphabetisch (A-Z)
    shiftDefsForStandort.sort((a, b) => {
      ...
      const isNumA = !isNaN(numA) && numA >= 1 && numA <= 100 && String(numA) === nameA;
      const isNumB = !isNaN(numB) && numB >= 1 && numB <= 100 && String(numB) === nameB;
      ...
      // Beide sind Text: alphabetische Sortierung
      const nameCompare = nameA.localeCompare(nameB, 'de', { sensitivity: 'base' });
      if (nameCompare !== 0) return nameCompare;
      
      // Bei gleichem Namen nach ID sortieren
...
// ---------------------------------------------------------
// Kommentar-Funktionen (müssen vor renderPersonnelSlotHTML definiert werden)
// ---------------------------------------------------------

// Globale Variablen für Kommentar-Popup
let kommentarPopupData = null; // { dayId, standortId, shiftId, slotIndex }
let hoverTooltipTimer = null;
let hoverTooltipElement = null;

// Kommentar-Popup öffnen
window.openKommentarPopup = async function (event, dayId, standortId, shiftId, slotIndex, mitarbeiterId) {
  try {
    if (event) {
    ...
    // Daten für Popup speichern
    kommentarPopupData = { dayId, standortId, shiftId, slotIndex };

    // Popup-Elemente
    let overlay = document.getElementById("kommentarPopupOverlay");
    let form = document.getElementById("kommentarPopupForm");
    const textarea = document.getElementById("kommentarText");

    if (!overlay || !form || !textarea) {
      console.error("Kommentar-Popup-Elemente nicht gefunden");
      return;
    }

    // WICHTIG: Stelle sicher, dass Overlay und Form direkt zu document.body gehören
    // Falls sie innerhalb eines anderen Popups sind, verschiebe sie zu body
    ...
    // Stelle sicher, dass z-index explizit gesetzt ist
    overlay.style.zIndex = "4000";
    form.style.zIndex = "4001";

    // Aktuellen Kommentar laden
    const currentKommentar = slotData.kommentar || "";
    textarea.value = currentKommentar;

    // Popup zuerst versteckt anzeigen, um getBoundingClientRect() zu ermöglichen
    overlay.style.display = "block";
    form.style.display = "block";
    form.style.visibility = "hidden"; // Versteckt, aber im DOM für getBoundingClientRect
    form.style.opacity = "0"; // Zusätzlich unsichtbar machen
    
    // Prüfe ob es ein mobiles Gerät ist (Tablet oder Handy)
    // Erhöhe die Schwelle auf 1200px, um auch Tablets zu erfassen
    ...
    if (!isMobile && event && event.target) {
      // Auf Desktop: Popup relativ zum angeklickten Element positionieren
      const targetElement = event.target.closest('.personnel-label');
      if (targetElement) {
        const rect = targetElement.getBoundingClientRect();
        
        // Warte kurz, damit das Popup gerendert wird, dann Position berechnen
        setTimeout(() => {
          const formRect = form.getBoundingClientRect();
          
          // Popup oberhalb des Elements positionieren
          let top = rect.top - formRect.height - 10; // 10px Abstand nach oben
          let left = rect.left + (rect.width / 2) - (formRect.width / 2); // Horizontal zentriert
          
          // Falls das Popup außerhalb des Viewports wäre, nach unten verschieben
          const viewportWidth = window.innerWidth;
          
          // Prüfe ob oben genug Platz ist, sonst nach unten
          if (top < 10) {
            top = rect.bottom + 10; // Unterhalb des Elements
          }
          
          // Prüfe ob links genug Platz ist
          if (left < 10) {
            left = 10;
          } else if (left + formRect.width > viewportWidth - 10) {
            left = viewportWidth - formRect.width - 10;
          }
          
          form.style.top = `${top}px`;
          form.style.left = `${left}px`;
          form.style.transform = 'none'; // Transform zurücksetzen, da wir absolute Position verwenden
          form.style.visibility = "visible"; // Jetzt sichtbar machen
          form.style.opacity = "1"; // Sichtbar machen
          
          // Textarea fokussieren
          textarea.focus();
        }, 10);
        return; // Früh zurückkehren, damit der Rest nicht ausgeführt wird
      }
    }
    
    // Auf mobilen Geräten oder Fallback: Immer zentriert
    if (isMobile) {
      // Auf mobilen Geräten: Zentriert positionieren
      // WICHTIG: Setze die Styles explizit, da inline-Styles höhere Priorität haben als CSS
      ...
      form.style.margin = '0';
      form.style.width = '95vw';
      form.style.maxWidth = '95vw';
    } else {
      // Fallback für Desktop, wenn kein Event vorhanden
      form.style.top = '50%';
      form.style.left = '50%';
      form.style.transform = 'translate(-50%, -50%)';
    }
    ...
    console.error("Fehler beim Öffnen des Kommentar-Popups:", e);
    alert("Fehler beim Öffnen des Kommentar-Popups: " + e.message);
  }
};

// Kommentar-Popup schließen
window.closeKommentarPopup = function () {
  const overlay = document.getElementById("kommentarPopupOverlay");
  const form = document.getElementById("kommentarPopupForm");
  const textarea = document.getElementById("kommentarText");
  ...
  kommentarPopupData = null;
};

// Kommentar speichern
async function saveKommentar() {
  try {
    if (!kommentarPopupData) {
      console.error("Keine Kommentar-Daten vorhanden");
      return;
    }

    const { dayId, standortId, shiftId, slotIndex } = kommentarPopupData;
    const textarea = document.getElementById("kommentarText");
    if (!textarea) return;
    ...
    const snap = await getDoc(ref);
    if (!snap.exists()) {
      console.error("Schicht-Dokument existiert nicht");
      return;
    }
    ...
    const slotData = data[field];
    if (!slotData) {
      console.error("Slot-Daten nicht gefunden");
      return;
    }

    // Kommentar zum Slot hinzufügen
    const updatedSlot = {
      ...slotData,
      kommentar: kommentar || null, // null wenn leer, damit es gelöscht werden kann
    };

    // In Firestore speichern
    await setDoc(ref, { [field]: updatedSlot }, { merge: true });

    console.log(`✅ Kommentar für Slot ${slotIndex} gespeichert`);
    ...
    alert("Fehler beim Speichern des Kommentars: " + e.message);
  }
}

// Hover-Handler für Tooltip
window.handlePersonnelSlotHover = function (event) {
  try {
    const element = event.currentTarget;
    if (!element) {
      console.log("🔍 handlePersonnelSlotHover: element nicht gefunden");
      return;
    }

    const kommentar = element.getAttribute("data-kommentar");
    console.log("🔍 handlePersonnelSlotHover: kommentar =", kommentar);
    
    if (!kommentar || kommentar.trim().length === 0) {
      console.log("🔍 handlePersonnelSlotHover: kein Kommentar vorhanden");
      return;
    }

    // Element und Position vor dem Timeout speichern, da event.currentTarget nach dem Timeout nicht mehr verfügbar ist
    ...
    // Timer für 1 Sekunde Delay
    hoverTooltipTimer = window.setTimeout(() => {
      console.log("🔍 handlePersonnelSlotHover: Timer abgelaufen, zeige Tooltip");
      showKommentarTooltip(elementData, kommentar);
    }, 1000);
  } catch (e) {
    console.error("Fehler bei handlePersonnelSlotHover:", e);
  }
};

// Hover-End-Handler
window.handlePersonnelSlotHoverEnd = function (event) {
  try {
    // Timer löschen
    ...
    console.error("Fehler bei handlePersonnelSlotHoverEnd:", e);
  }
};

// Tooltip anzeigen
function showKommentarTooltip(elementData, kommentar) {
  try {
    console.log("🔍 showKommentarTooltip aufgerufen, kommentar =", kommentar);
    
    // Alten Tooltip entfernen, falls vorhanden
    if (hoverTooltipElement) {
      hoverTooltipElement.remove();
      hoverTooltipElement = null;
    }
    ...
    tooltip.className = "kommentar-tooltip";
    tooltip.textContent = kommentar;
    
    // Tooltip zuerst versteckt hinzufügen, damit getBoundingClientRect() funktioniert
    ...
    // WICHTIG: Tooltip direkt zu document.body hinzufügen, nicht zu einem Container
    document.body.appendChild(tooltip);
    hoverTooltipElement = tooltip;
    
    console.log("🔍 Tooltip erstellt und zu body hinzugefügt");

    // Position aus gespeicherten Daten verwenden
    if (!elementData || !elementData.rect) {
      console.error("elementData nicht gefunden");
      return;
    }
    ...
    // Warte kurz, damit der Tooltip gerendert wird
    setTimeout(() => {
      if (!tooltip.parentNode) {
        console.log("🔍 Tooltip wurde bereits entfernt");
        return; // Tooltip wurde bereits entfernt
      }
      
      const tooltipRect = tooltip.getBoundingClientRect();
      console.log("🔍 Tooltip-Größe:", tooltipRect);
      
      const viewportWidth = window.innerWidth;
      const viewportHeight = window.innerHeight;
      
      // Tooltip horizontal zentriert über dem Element positionieren
      let left = rect.left + (rect.width / 2) - (tooltipRect.width / 2);
      let top = rect.top - tooltipRect.height - 10; // 10px Abstand nach oben
      ...
      // Prüfe ob oben genug Platz ist, sonst nach unten verschieben
      if (top < 10) {
        top = rect.bottom + 10; // Unterhalb des Elements
      }
      ...
      // Position setzen - WICHTIG: px-Einheiten explizit setzen
      tooltip.style.left = `${Math.round(left)}px`;
      tooltip.style.top = `${Math.round(top)}px`;
      tooltip.style.visibility = "visible";
      tooltip.style.display = "block";
      
      console.log(`✅ Tooltip positioniert: left=${Math.round(left)}px, top=${Math.round(top)}px, Element: left=${rect.left}px, top=${rect.top}px, width=${rect.width}px`);
    }, 10);
  } catch (e) {
    console.error("Fehler beim Anzeigen des Tooltips:", e);
  }
}
...
function renderPersonnelSlotHTML(dayId, standortId, shiftId, slotIndex, personalData) {
  ...
  const kommentar = personalData.kommentar || "";
  const hasKommentar = kommentar.trim().length > 0;
  
  return `
    <div class="personnel-label assigned ${hasKommentar ? 'has-kommentar' : ''}"
         style="background-color: ${bgColor};"
         data-day-id="${dayId}"
         data-standort-id="${standortId}"
         data-shift-id="${shiftId}"
         data-slot-index="${slotIndex}"
         data-mitarbeiter-id="${personalData.mitarbeiterId || ""}"
         data-kommentar="${kommentar.replace(/"/g, '&quot;')}"
         onclick="handlePersonnelSlotClick(event,'${dayId}','${standortId}','${shiftId}',${slotIndex},'${personalData.mitarbeiterId || ""}')"
         oncontextmenu="openKommentarPopup(event,'${dayId}','${standortId}','${shiftId}',${slotIndex},'${personalData.mitarbeiterId || ""}')"
         ...
         onmouseenter="handlePersonnelSlotHover(event)"
         onmouseleave="handlePersonnelSlotHoverEnd(event)">
...
window.handlePersonnelSlotClick = function (event, dayId, standortId, shiftId, slotIndex, mitarbeiterId) {
  try {
    // Nicht öffnen, wenn Long-Press ausgelöst wurde
    if (personnelLongPressTriggered) {
      personnelLongPressTriggered = false;
      return;
    }
    
    // Nicht öffnen, wenn auf den Lösch-Button geklickt wurde
    if (event && event.target && event.target.closest('.personnel-delete-btn')) {
      return;
    }
...
window.openMitarbeiterDatenblattFromSlot = async function (
  ...
  try {
    ...
    console.log("✅ Öffne Mitarbeiter-Datenblatt für:", formatMitarbeiterName(m.vorname, m.nachname));
    ...
    // Handynummer als tel: Link formatieren
    const handynummer = m.handynummer || m.handy || "-";
    const handyLink = handynummer && handynummer !== "-" ? handynummer.replace(/\s+/g, "") : "";
    const handynummerDisplay = handyLink
      ? `<a href="tel:${handyLink}" style="color: var(--primary-color); text-decoration: underline;">${handynummer}</a>`
      : "-";

    // Kommentar aus Slot-Daten lesen
    const kommentar = slotData.kommentar || "";
    ...
    content.innerHTML = `
      <div class="mitarbeiter-datenblatt">
        <div class="mitarbeiter-datenblatt-content">
          <div class="mitarbeiter-datenblatt-info">
            <h3>${formatMitarbeiterName(m.vorname, m.nachname)}</h3>
            <p><strong>Qualifikation:</strong> ${qualis || "-"}</p>
            <p><strong>Führerschein:</strong> ${fuehrerschein}</p>
            <p><strong>Telefon:</strong> ${telefonDisplay}</p>
            <p><strong>Handynummer:</strong> ${handynummerDisplay}</p>
          </div>
          <div class="mitarbeiter-datenblatt-kommentar">
            <label for="mitarbeiterKommentarTextarea">Kommentar:</label>
            <textarea id="mitarbeiterKommentarTextarea" rows="6" placeholder="Kommentar eingeben...">${kommentar.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</textarea>
          </div>
        </div>
        ...
        <div class="form-actions">
          <button id="saveSlotColorBtn" style="padding: 10px 15px; background-color: var(--primary-color); color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600;">Speichern</button>
          <button onclick="closeMitarbeiterDatenblatt()" class="cancel-btn">Schließen</button>
        </div>
      </div>
    `;

    // Speichern-Button Handler (speichert Farbe + Kommentar)
    const saveBtn = document.getElementById("saveSlotColorBtn");
    if (saveBtn) {
      saveBtn.addEventListener("click", async () => {
        const colorSelect = document.getElementById("slotColorSelect");
        const kommentarTextarea = document.getElementById("mitarbeiterKommentarTextarea");
        if (!colorSelect) return;

        const newColor = colorSelect.value || "#ffffff";
        const newKommentar = kommentarTextarea ? kommentarTextarea.value.trim() : "";
        
        try {
          const updatedSlot = {
            ...slotData,
            farbe: newColor,
            kommentar: newKommentar || null, // null wenn leer, damit es gelöscht werden kann
          };
          await setDoc(ref, { [field]: updatedSlot }, { merge: true });
          console.log(`✅ Farbe und Kommentar für Slot ${slotIndex} gespeichert`);
...
// Long-Press auf Touch-Geräten: Kommentar-Popup öffnen, Copy unterdrücken
let personnelLongPressTimer = null;
let personnelLongPressTriggered = false; // Flag, um zu verhindern, dass normaler Click nach Long-Press ausgelöst wird
...
window.personnelSlotTouchStart = function (event, dayId, standortId, shiftId, slotIndex, mitarbeiterId) {
  try {
    ...
    personnelLongPressTimer = window.setTimeout(() => {
      console.log("🔍 Long-Press erkannt, öffne Kommentar-Popup");
      personnelLongPressTriggered = true;
      ...
      // Kommentar-Popup öffnen
      openKommentarPopup(event, dayId, standortId, shiftId, slotIndex, mitarbeiterId);
...
window.personnelSlotTouchEnd = function (event) {
  try {
    ...
    // Wenn Long-Press ausgelöst wurde, verhindere normalen Click
    if (personnelLongPressTriggered && event) {
      event.preventDefault();
      event.stopPropagation();
      personnelLongPressTriggered = false;
    }
...
function fillMitarbeiterSelect() {
  ...
  try {
    ...
    mitarbeiterArray.forEach((m) => {
      ...
      opt.textContent = `${formatMitarbeiterNameForSelect(m.vorname, m.nachname)}${details}`;
    ...
    if (personnelMitarbeiterSearch) {
      personnelMitarbeiterSearch.oninput = () => {
        ...
        const filtered = allMitarbeiter
          ...
          .filter((m) =>
            !term ||
            (m.nachname && m.nachname.toLowerCase().includes(term)) ||
            (m.vorname && m.vorname.toLowerCase().includes(term))
          );
...
function renderMitarbeiterList(mitarbeiterArray, searchTerm) {
  ...
  mitarbeiterArray.forEach((m) => {
    ...
    item.textContent = `${formatMitarbeiterNameForSelect(m.vorname, m.nachname)}${details}`;
...
if (savePersonnelBtn) {
  savePersonnelBtn.addEventListener("click", async () => {
    ...
    const personalData = {
      mitarbeiterId,
      name: formatMitarbeiterName(mitarbeiter.vorname, mitarbeiter.nachname),
...
function setupSettingsHandlers() {
  ...
  // Zugriff auf Standorte, Schichten und Bereitschaften-Verwaltung haben: Superadmin, Admin, Rettungsdienstleiter, Wachleitung
  const allowedRolesForSettings = ["superadmin", "admin", "rettungsdienstleiter", "wachleitung"];
  const isAllowed = userAuthData && allowedRolesForSettings.includes(userAuthData.role);
  ...
  const schichtSchichtartInput = document.getElementById("schichtSchichtart");
  ...
  if (addSchichtBtn && schichtForm && schichtNameInput && schichtStandortSelect && schichtFormTitle && saveSchichtBtn) {
    ...
    saveSchichtBtn.addEventListener("click", async () => {
      ...
      const schichtart = schichtSchichtartInput ? (schichtSchichtartInput.value || "").trim() : "";
      
      if (!name || !standortId || !schichtart) {
        alert("Bitte Schichtname, Standort und Schichtart auswählen.");
        return;
      }

      console.log(`💾 Speichere Schicht: name="${name}", standortId="${standortId}", schichtart="${schichtart}"`);
      ...
      try {
        ...
        const dataToSave = {
          ...
          schichtart: schichtart, // Schichtart ist jetzt Pflichtfeld
  ...
  if (
    ...
    saveMitarbeiterBtn.addEventListener("click", async () => {
      ...
      const fuehrerschein = mitarbeiterFuehrerscheinInput
        ...
        : "";
      const telefon = mitarbeiterTelefonInput ? mitarbeiterTelefonInput.value.trim() : "";

      try {
        // 🔥 NEUE ZENTRALE MITARBEITER-COLLECTION: Verwende neue Feldnamen
        // telefon statt telefonnummer (für Kompatibilität beim Laden beide unterstützen)
        const payload = {
          vorname,
          nachname,
          name: formatMitarbeiterName(vorname, nachname), // Vollständiger Name für Anzeige
          qualifikation: qualis,
          fuehrerschein,
          telefon, // NEU: telefon statt telefonnummer
          active: true,
        };
...
// ---------------------------------------------------------
// Offene Dienste Popup Handler
// ---------------------------------------------------------

function setupOpenShiftsHandler() {
  if (!openShiftsBtn || !openShiftsPopupOverlay || !openShiftsPopupForm) return;

  // Verhindere doppelte Event-Listener
  if (openShiftsBtn.dataset.listenerAdded) {
    console.log("⚠️ setupOpenShiftsHandler bereits aufgerufen, überspringe");
    return;
  }

  openShiftsBtn.addEventListener("click", async () => {
    openShiftsPopupOverlay.style.display = "block";
    openShiftsPopupForm.style.display = "flex";
  });
  openShiftsBtn.dataset.listenerAdded = "true";

  // Event-Listener für Schichtart-Auswahl (nur einmal registrieren)
  if (openShiftsSchichtartSelect && !openShiftsSchichtartSelect.dataset.listenerAdded) {
    openShiftsSchichtartSelect.addEventListener("change", async () => {
      await loadAndDisplayOpenShifts();
    });
    openShiftsSchichtartSelect.dataset.listenerAdded = "true";
  }

  // Schließen bei Klick auf Overlay (nur einmal registrieren)
  if (!openShiftsPopupOverlay.dataset.listenerAdded) {
    openShiftsPopupOverlay.addEventListener("click", (e) => {
      if (e.target === openShiftsPopupOverlay) {
        closeOpenShiftsPopup();
      }
    });
    openShiftsPopupOverlay.dataset.listenerAdded = "true";
  }
}


window.closeOpenShiftsPopup = function () {
  // Setze Auswahl zurück
  if (openShiftsSchichtartSelect) {
    openShiftsSchichtartSelect.value = "";
  }
  
  // Leere Textfeld
  if (openShiftsTextarea) {
    openShiftsTextarea.value = "";
  }
  
  if (openShiftsPopupOverlay) openShiftsPopupOverlay.style.display = "none";
  if (openShiftsPopupForm) openShiftsPopupForm.style.display = "none";
};

window.copyOpenShifts = function () {
  if (!openShiftsTextarea) return;
  
  openShiftsTextarea.select();
  openShiftsTextarea.setSelectionRange(0, 99999); // Für mobile Geräte
  
  try {
    document.execCommand('copy');
    // Zeige kurze Bestätigung
    const copyBtn = document.querySelector('#openShiftsPopupForm .primary-btn');
    if (copyBtn) {
...
/**
 * Lädt alle offenen Dienste für den aktuellen Monat und zeigt sie formatiert an
 */
async function loadAndDisplayOpenShifts() {
  if (!openShiftsTextarea || !openShiftsSchichtartSelect) return;

  const selectedSchichtart = openShiftsSchichtartSelect.value;
  
  if (!selectedSchichtart) {
    openShiftsTextarea.value = "Bitte wählen Sie eine Art aus (KTW-Schichten oder RTW-Schichten).";
    return;
  }

  // Stelle sicher, dass Standorte und Schichten geladen sind
  if (!allStandorte || allStandorte.length === 0) {
    await loadStandorte();
  }
  ...
  // Filtere Schichten nach Schichtart
  // KTW-Schichten: Schichten mit Schichtart "KTW" oder "N-KTW"
  // RTW-Schichten: Schichten mit Schichtart "RTW", "ITW" oder "S-RTW"
  const schichtartMapping = {
    "KTW": ["KTW", "N-KTW"],
    "RTW": ["RTW", "ITW", "S-RTW"]
  };
  
  const allowedSchichtarten = schichtartMapping[selectedSchichtart] || [];
  const filteredSchichten = allSchichten.filter(s => {
    if (!s.schichtart) return false; // Nur Schichten mit Schichtart
    return allowedSchichtarten.includes(s.schichtart);
  });

  if (filteredSchichten.length === 0) {
    openShiftsTextarea.value = `Keine Schichten mit Schichtart "${selectedSchichtart === "KTW" ? "KTW-Schichten" : "RTW-Schichten"}" gefunden.`;
    return;
  }

  // Erstelle Sets für schnellen Zugriff: IDs und Namen der gefilterten Schichten
  const filteredSchichtIds = new Set(filteredSchichten.map(s => s.id));
  const filteredSchichtNames = new Set(filteredSchichten.map(s => s.name).filter(n => n));

  // Hole alle Standorte, die zu den gefilterten Schichten gehören
  const standortIds = new Set(filteredSchichten.map(s => s.standortId));
  const filteredStandorte = allStandorte.filter(s => {
    if (s.active === false) return false;
    return standortIds.has(s.id);
  });

  console.log(`🔍 Gefilterte Schichten: ${filteredSchichten.length} Schichten gefunden`);
  console.log(`🔍 Gefilterte Standorte: ${filteredStandorte.length} Standorte gefunden`);
  console.log(`🔍 Schicht-IDs:`, Array.from(filteredSchichtIds));
  console.log(`🔍 Schicht-Namen:`, Array.from(filteredSchichtNames));

  openShiftsTextarea.value = "Lade offene Dienste...";
  ...
  try {
    ...
    // Struktur: { dayId: [schichtName1, schichtName2, ...] }
    const openShiftsByDate = {};

    // Durchlaufe alle Tage des Monats
    for (let day = 1; day <= daysInMonth; day++) {
      const date = new Date(currentYear, currentMonth, day);
      const dayId = formatDayId(date); // Format: DD.MM.YYYY

      // Durchlaufe alle gefilterten Standorte
      for (const standort of filteredStandorte) {
        try {
          // Lade Schichten für diesen Tag und Standort
          const shiftsCol = getShiftsCollection(standort.id, dayId);
          const shiftsSnap = await getDocs(shiftsCol);
          ...
          if (!shiftsSnap.empty) {
            shiftsSnap.forEach((docSnap) => {
              ...
              // Prüfe, ob diese Schicht zu den gefilterten Schichten gehört
              // Die Schicht-ID kann entweder docSnap.id (ID der Schicht-Definition) oder
              // der Schichtname sein (falls im Tages-Dokument als shiftName gespeichert)
              const shiftId = docSnap.id;
              const shiftNameFromData = data.shiftName;
              
              // Prüfe nach ID oder Name
              const isFilteredSchicht = 
                filteredSchichtIds.has(shiftId) || 
                (shiftNameFromData && filteredSchichtNames.has(shiftNameFromData)) ||
                filteredSchichtNames.has(shiftId);
              
              if (!isFilteredSchicht) {
                return; // Diese Schicht gehört nicht zu den gefilterten Schichten
              }
              ...
              // Wenn Schicht nicht vollständig bemannt ist
              if (!fullyManned) {
                const shiftName = getShiftDisplayName(standort.id, shiftId, data) || shiftNameFromData || shiftId;
                
                // Initialisiere Array für Datum, falls noch nicht vorhanden
                if (!openShiftsByDate[dayId]) {
                  openShiftsByDate[dayId] = [];
                }

                // Füge Schicht hinzu, wenn noch nicht vorhanden (vermeide Duplikate)
                if (!openShiftsByDate[dayId].includes(shiftName)) {
                  openShiftsByDate[dayId].push(shiftName);
                }
              }
            });
          }
        } catch (err) {
          console.error(`Fehler beim Laden für Standort ${standort.name}, Tag ${dayId}:`, err);
        }
      }
    }

    // Formatiere Ausgabe mit spezifischem Text je nach Schichtart
    let output = "";
    if (selectedSchichtart === "KTW") {
      output = "Hallo zusammen, \nhier die aktuellen offenen KTW-Dienste\n\n";
    } else if (selectedSchichtart === "RTW") {
      output = "Hallo zusammen, \nhier die aktuellen offenen RTW-Dienste\n\n";
    } else {
      output = "Hier die aktuellen offenen Dienste:\n\n";
    }
    
    // Sortiere Daten chronologisch
    const sortedDates = Object.keys(openShiftsByDate).sort((a, b) => {
      // Konvertiere DD.MM.YYYY zu Date für Vergleich
    ...
    if (sortedDates.length === 0) {
      if (selectedSchichtart === "KTW") {
        output = "Hallo zusammen, \nhier die aktuellen offenen KTW-Dienste\n\nKeine offenen Dienste im angezeigten Monat.\n\nWenn Ihr uns unterstützen könnt, meldet Euch bitte beim OVD oder den Wachleitungen.";
      } else if (selectedSchichtart === "RTW") {
        output = "Hallo zusammen, \nhier die aktuellen offenen RTW-Dienste\n\nKeine offenen Dienste im angezeigten Monat.\n\nWenn Ihr uns unterstützen könnt, meldet Euch bitte beim OVD oder den Wachleitungen.";
      } else {
        output = `Hier die aktuellen offenen Dienste:\n\nKeine offenen Dienste im angezeigten Monat für ${selectedSchichtart === "KTW" ? "KTW-Schichten" : "RTW-Schichten"}.`;
      }
    } else {
      sortedDates.forEach((dayId) => {
        const shifts = openShiftsByDate[dayId];
        
        // Sortiere Schichten: Zahlen (1-100) zuerst, dann alphabetisch
        shifts.sort((a, b) => {
          const numA = parseInt(a.trim(), 10);
          const numB = parseInt(b.trim(), 10);
          const isNumA = !isNaN(numA) && numA >= 1 && numA <= 100 && String(numA) === a.trim();
          const isNumB = !isNaN(numB) && numB >= 1 && numB <= 100 && String(numB) === b.trim();
          
          if (isNumA && isNumB) return numA - numB;
          if (isNumA) return -1;
          if (isNumB) return 1;
          return a.trim().localeCompare(b.trim(), 'de', { sensitivity: 'base' });
        });

        // Für jede Schicht: Zeige "Datum : Schicht"
        shifts.forEach((shiftName) => {
          output += `${dayId} : ${shiftName}\n`;
        });
      });
      
      // Füge Abschlusstext hinzu
      output += "\nWenn Ihr uns unterstützen könnt, meldet Euch bitte beim OVD oder den Wachleitungen.";
    }

    openShiftsTextarea.value = output;
  } catch (error) {
    console.error("Fehler beim Laden der offenen Dienste:", error);
    openShiftsTextarea.value = "Fehler beim Laden der offenen Dienste.";
  }
}
    ...
    if (standorteList) {
      ...
      allStandorte.forEach((s, index) => {
        ...
        editBtn.addEventListener("click", () => {
          ...
          const standortFormTitle = document.getElementById("standortFormTitle");
          const standortNameInput = document.getElementById("standortName");
          const standortSchichtartInput = document.getElementById("standortSchichtart");
          const ov = document.getElementById("standortFormOverlay");
          ...
          if (standortSchichtartInput) {
            standortSchichtartInput.value = s.schichtart || "";
          }
...
async function reloadSchichtenSettings(schichtenList) {
  ...
  try {
    ...
    // Sortiere Schichten: zuerst nach Standortname (alphabetisch), dann nach Schichtname (Zahlen 1-100, dann A-Z)
    const sortedSchichten = [...allSchichten].sort((a, b) => {
      ...
      // Bei gleichem Standort nach Schichtname sortieren: Zahlen (1-100) zuerst, dann alphabetisch (A-Z)
      const nameA = (a.name || "").trim();
      const nameB = (b.name || "").trim();
    ...
    sortedSchichten.forEach((s) => {
      ...
      editBtn.addEventListener("click", async () => {
        ...
        if (schichtSchichtartInput) {
          schichtSchichtartInput.value = s.schichtart || "";
        }
  ...
  try {
    ...
    const filteredMitarbeiter = searchLower 
      ? sortedMitarbeiter.filter(m => {
          const fullName = formatMitarbeiterName(m.vorname, m.nachname).toLowerCase();
    ...
    filteredMitarbeiter.forEach((m) => {
      ...
      editBtn.addEventListener("click", () => {
        ...
        // Kompatibilität: Unterstütze sowohl telefon als auch telefonnummer
        if (mitarbeiterTelefonInput) mitarbeiterTelefonInput.value = m.telefon || m.telefonnummer || "";
...
async function openMitarbeiterDatenblattFromBereitschaft(dayId, bereitschaft) {
  ...
  try {
    ...
    if (offeneSlots.length === 0) {
      schichtzuweisungHTML = '<p style="color: #64748b; font-style: italic; margin-top: 20px;">Keine offenen Schicht-Slots an diesem Tag verfügbar.</p>';
    } else {
      ...
      if (zuordnenBtn && select) {
        zuordnenBtn.addEventListener("click", async () => {
          ...
          const personalData = {
            ...
            name: bereitschaft.name || (m ? formatMitarbeiterName(m.vorname, m.nachname) : "Unbekannt"),
            qualifikationen: qualis,
  ...
  try {
    ...
    confirmBtn.addEventListener("click", async () => {
      ...
      const personalData = {
        mitarbeiterId: bereitschaft.mitarbeiterId,
        name:
          bereitschaft.name || (mitarbeiter ? formatMitarbeiterName(mitarbeiter.vorname, mitarbeiter.nachname) : "Unbekannt"),
        qualifikationen: qualis,
...
window.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    // Kommentar-Popup Event Listener
    const kommentarPopupOverlay = document.getElementById("kommentarPopupOverlay");
    const saveKommentarBtn = document.getElementById("saveKommentarBtn");
    
    if (kommentarPopupOverlay) {
      kommentarPopupOverlay.addEventListener("click", (e) => {
        if (e.target === kommentarPopupOverlay) {
          closeKommentarPopup();
        }
      });
    }
    ...
    if (saveKommentarBtn) {
      saveKommentarBtn.addEventListener("click", async () => {
        await saveKommentar();
      });
    }
