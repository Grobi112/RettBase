...
/**
 * Cloud Function zum Aktualisieren des Passworts eines Mitarbeiters
 * Wird von der Mitgliederverwaltung aufgerufen, wenn Admin das Passwort zurücksetzt
 * Unterstützt: { uid, newPassword } oder { email, newPassword } (falls Account existiert, aber keine UID in Firestore)
 */
exports.updateMitarbeiterPassword = functions.region("us-central1").https.onCall(async (data, context) => {
  if (!context || !context.auth) {
  ...
  try {
    ...
    console.log("✅ Passwort aktualisiert für UID:", targetUid);
    return { success: true, uid: targetUid };
  } catch (error) {
    console.error("❌ updateMitarbeiterPassword:", error);
