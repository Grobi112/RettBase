...
function initializeRichTextEditor() {
  ...
  // Setze Sans Serif als Standard-Schriftart und normal als Standard-Größe
  if (quillEditor) {
    // Setze Standard-Schriftart im Editor-Container
    const editorElement = quillEditor.root;
    if (editorElement) {
      editorElement.style.fontFamily = 'sans-serif';
    }
  }
  
  // Enter-Handler: Standard-Verhalten (keine spezielle Formatierung nötig, da Size/Font fest sind)
...
// ALTE FUNKTIONEN ENTFERNT - Wird nicht mehr benötigt, da wir Quill.js verwenden
// executeFormatCommand und updateToolbarButtons wurden entfernt, da Quill.js die Formatierung selbst verwaltet

// syncEditorToTextarea - Synchronisiert Quill-Inhalt mit verstecktem Textarea
function syncEditorToTextarea() {
  if (quillEditor && emailBodyHidden) {
    emailBodyHidden.value = quillEditor.root.innerHTML;
  }
}
...
async function loadUsers() {
  try {
    ...
    console.log(`🔍 loadUsers: companyId = "${companyId}"`);
    ...
    // 🔥 WICHTIG: Setze allUsers zurück, um sicherzustellen, dass keine alten Daten von anderen Firmen übrig bleiben
    allUsers = [];
    
    // 🔥 NEUE ZENTRALE MITARBEITER-COLLECTION: Lade alle Mitarbeiter aus mitarbeiter Collection
    // Die Collection enthält jetzt alle Daten: Login-Daten, Rollen, E-Mail-Adressen, etc.
    const mitarbeiterRef = collection(db, "kunden", companyId, "mitarbeiter");
    const mitarbeiterSnapshot = await getDocs(mitarbeiterRef);
    console.log(`👥 ${mitarbeiterSnapshot.size} Mitarbeiter aus zentraler mitarbeiter Collection für Firma "${companyId}" geladen`);
    
    // Verarbeite alle Mitarbeiter
    mitarbeiterSnapshot.forEach((doc) => {
      ...
      // Name ermitteln: name, oder vorname + nachname
      let fullName = mitarbeiterData.name || "";
      if (!fullName && (mitarbeiterData.vorname || mitarbeiterData.nachname)) {
        fullName = `${mitarbeiterData.vorname || ""} ${mitarbeiterData.nachname || ""}`.trim();
      }
      // Fallback: E-Mail oder "Unbekannt"
      if (!fullName || fullName.trim() === "") {
        fullName = mitarbeiterData.email || mitarbeiterData.internalEmail || "Unbekannt";
      }
      
      // E-Mail-Adresse: Für E-Mail-Modul verwenden wir internalEmail, falls vorhanden, sonst email
      const email = mitarbeiterData.internalEmail || mitarbeiterData.email || "";
      
      const user = {
        uid: mitarbeiterData.uid || doc.id, // UID aus Daten oder Dokument-ID
        name: fullName,
        email: email,
        role: mitarbeiterData.role || "",
        ...
        loginEmail: mitarbeiterData.email || "", // Login-E-Mail (kann unterschiedlich von internalEmail sein)
        // 🔥 SICHERHEIT: Speichere companyId im User-Objekt zur Validierung
        _companyId: companyId
      };
      
      console.log(`👤 Mitarbeiter geladen: ${user.name} (${user.email}) - UID: ${user.uid} - CompanyId: ${companyId}`);
      console.log(`   📋 Mitarbeiter-Daten: name="${fullName}", vorname="${mitarbeiterData.vorname}", nachname="${mitarbeiterData.nachname}", email="${mitarbeiterData.email}", internalEmail="${mitarbeiterData.internalEmail}"`);
      
      allUsers.push(user);
    });
    
    // 🔥 SICHERHEIT: Finale Validierung - entferne alle Benutzer, die nicht zur aktuellen Firma gehören
    const currentCompanyId = getCompanyId();
    const initialLength = allUsers.length;
    allUsers = allUsers.filter(user => {
      // Prüfe explizit die companyId, falls vorhanden, oder vertraue auf die Collection-Struktur
      if (user._companyId && user._companyId !== currentCompanyId) {
        console.warn(`⚠️ Entferne Benutzer aus falscher Firma: ${user.name} (${user.email}) - erwartet: ${currentCompanyId}, gefunden: ${user._companyId}`);
        return false;
      }
      return true;
    });
    ...
    if (allUsers.length !== initialLength) {
      console.warn(`⚠️ ${initialLength - allUsers.length} Benutzer entfernt, die nicht zur Firma "${currentCompanyId}" gehören`);
    }
    
    console.log(`✅ ${allUsers.length} Benutzer geladen (inkl. Mitarbeiter ohne Login) für Firma "${currentCompanyId}"`);
  } catch (error) {
    console.error("❌ Fehler beim Laden der Benutzer:", error);
    // 🔥 SICHERHEIT: Bei Fehler allUsers leeren, um keine veralteten Daten zu behalten
    allUsers = [];
  }
}

// ---------------------------------------------------------
// E-Mail-Gruppen laden
// ---------------------------------------------------------
...
function fillMitarbeiterSelect() {
  ...
  console.log(`🔍 fillMitarbeiterSelect aufgerufen. allUsers.length = ${allUsers.length}`);
  ...
  if (mitarbeiterSelect && !isMobile) {
    ...
    console.log(`📋 Fülle Select-Dropdown mit ${allUsers.length} Benutzern`);
    allUsers.forEach((user, index) => {
      ...
      option.textContent = user.name || "Unbekannt";
      option.dataset.uid = user.uid || "";
      mitarbeiterSelect.appendChild(option);
      console.log(`  ✅ Option hinzugefügt: ${option.textContent} (Index: ${index})`);
    });
...
function renderMitarbeiterList(searchTerm = "") {
  ...
  const filtered = allUsers.filter(user => {
    ...
    // Suche in Name, Vorname, Nachname, E-Mail und interner E-Mail
    const searchable = `${user.name || ""} ${user.vorname || ""} ${user.nachname || ""} ${user.email || ""} ${user.internalEmail || ""}`.toLowerCase();
...
function closeComposeModal() {
  ...
  if (composeModal) {
    ...
    if (quillEditor) {
      quillEditor.setContents([]);
      // Setze Arial als Standard-Schriftart (Arial ist eine sans-serif Schriftart)
...
// Upload-Funktion für Dateianhänge
async function uploadAttachments(companyId, userId, timestampId) {
  if (!emailAttachments || emailAttachments.length === 0) {
    return [];
  }

  const attachmentPromises = emailAttachments.map(async (attachment, index) => {
    try {
      // Erstelle eindeutigen Pfad: kunden/{companyId}/email-attachments/{timestampId}/{index}_{filename}
      const sanitizedName = attachment.name.replace(/[^a-zA-Z0-9.-]/g, '_');
      const filePath = `kunden/${companyId}/email-attachments/${timestampId}/${index}_${sanitizedName}`;
      const storageRef = ref(storage, filePath);

      // Upload Datei
      console.log(`📎 Lade Datei hoch: ${attachment.name} -> ${filePath}`);
      await uploadBytes(storageRef, attachment.file);

      // Hole Download-URL
      const downloadURL = await getDownloadURL(storageRef);
      console.log(`✅ Datei hochgeladen: ${attachment.name} -> ${downloadURL}`);
        ...
        try {
          ...
          const result = await sendEmail({
            ...
            attachments: uploadedAttachments.length > 0 ? uploadedAttachments.map(att => ({
              filename: att.name,
              path: att.url, // Download-URL von Firebase Storage
...
async function loadAllGroupMembers() {
  try {
    ...
    mitarbeiterSnapshot.forEach((doc) => {
      ...
      // Prüfe active oder status (Kompatibilität)
      if (userData.active !== false && userData.status !== false) {
        ...
        if (existingIndex !== -1) {
          ...
          allGroupMembers[existingIndex].uid = userData.uid || doc.id; // Verwende uid-Feld, falls vorhanden
          ...
          allGroupMembers.push({
            uid: userData.uid || doc.id, // Verwende uid-Feld, falls vorhanden
            name: name,
            vorname: vorname,
            nachname: nachname,
