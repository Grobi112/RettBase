...
let editAllowedRoles = ['superadmin', 'admin', 'rettungsdienstleitung']; // Rollen, die abgeschlossene/vergangene Tage bearbeiten dürfen
let settingsAllowedRoles = ['superadmin', 'admin', 'rettungsdienstleitung']; // Rollen, die Einstellungen sehen können
...
function canAccessSettings() {
  ...
  return settingsAllowedRoles.includes(userAuthData.role);
}
...
function canEditDay(dayDoc) {
  ...
  // Superadmin, Admin und Rettungsdienstleitung können immer bearbeiten
  if (editAllowedRoles.includes(userAuthData?.role)) {
    return true;
  }
 ...
 * Superadmin, Admin und Rettungsdienstleitung über das Einstellungsmenü möglich.
 */
...
async function openAddEvent() {
  ...
  // Für OVD-Rolle: Feld schreibgeschützt, für andere Rollen bearbeitbar
  if (userAuthData?.role === 'ovd') {
    eventOvdInput.readOnly = true;
    eventOvdInput.style.backgroundColor = '#f3f4f6';
  } else {
    eventOvdInput.readOnly = false;
    eventOvdInput.style.backgroundColor = 'white';
  }
...
async function openEditEvent(eventId) {
  ...
  try {
    ...
    // Beim Bearbeiten: Verwende den gespeicherten Wert oder lade den Namen des aktuellen Benutzers
    if (event.diensthabenderOvd) {
      eventOvdInput.value = event.diensthabenderOvd;
    } else {
      // Falls kein Wert gespeichert: Lade den Namen des eingeloggten Benutzers
      const employeeName = await getEmployeeName(userAuthData?.uid);
      eventOvdInput.value = employeeName || userAuthData?.email || "Unbekannt";
    }
...
eventForm.addEventListener('submit', async (e) => {
  ...
  // Verwende den Wert aus dem Input-Feld, falls leer oder "Unbekannt": Lade den Namen des eingeloggten Benutzers
  let diensthabenderOvd = eventOvdInput.value.trim();
  if (!diensthabenderOvd || diensthabenderOvd === "Unbekannt") {
    const employeeName = await getEmployeeName(userAuthData.uid);
    if (employeeName) {
      diensthabenderOvd = employeeName;
...
/**
 * Initialisiert das Modul für den aktuellen Tag
 */
