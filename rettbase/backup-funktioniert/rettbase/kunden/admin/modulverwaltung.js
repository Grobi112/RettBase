...
moduleForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        // Zusätzlich für alle anderen Firmen freischalten (wenn kostenlos)
        try {
            if (free) {
                const companiesRef = collection(db, "kunden");
                const companiesSnap = await getDocs(companiesRef);
                
                const enablePromises = [];
                companiesSnap.forEach(companyDoc => {
                    // Admin bereits oben freigeschaltet, daher überspringen
                    if (companyDoc.id !== "admin") {
                        enablePromises.push(setCompanyModule(companyDoc.id, moduleId, true));
                    }
                });
                
                if (enablePromises.length > 0) {
                    await Promise.all(enablePromises);
                    console.log(`✅ Modul '${moduleId}' zusätzlich für ${enablePromises.length} weitere Firmen freigeschaltet (kostenlos)`);
                }
            }
        } catch (enableError) {
            console.warn("⚠️ Fehler beim Freischalten für andere Firmen (nicht kritisch):", enableError);
        }
