...
import 'einsatzprotokoll_nfs_druck_screen.dart';
...
/// Protokollübersicht NFS – gespeicherte Einsatzprotokolle auflisten
class EinsatzprotokollNfsUebersichtScreen extends StatefulWidget {
  final String companyId;
  final String userRole;
  final VoidCallback onBack;

  const EinsatzprotokollNfsUebersichtScreen({
    super.key,
  ...
  @override
  State<EinsatzprotokollNfsUebersichtScreen> createState() =>
      _EinsatzprotokollNfsUebersichtScreenState();
}

enum _FilterMode { jahr, datum }

class _EinsatzprotokollNfsUebersichtScreenState
    extends State<EinsatzprotokollNfsUebersichtScreen> {
  _FilterMode _filterMode = _FilterMode.jahr;
  int _filterYear = DateTime.now().year;
  DateTime _filterDate = DateTime.now();
  final _service = EinsatzprotokollNfsService();
  ...
  List<int> get _years =>
      List.generate(DateTime.now().year - 2020 + 1, (i) => 2020 + i).reversed.toList();
  ...
  Future<void> _confirmDelete(BuildContext context, String docId, String einsatzNr) async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Möchten Sie Protokoll Nr. $einsatzNr wirklich unwiderruflich löschen?',
        ),
        actions: [
          TextButton(
        ...
        children: [
          ...
          Expanded(
            child: StreamBuilder<List<Map<String, dynamic>>>(
              stream: _service.streamProtokolle(widget.companyId),
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        title: Text(
                          einsatzNr.isNotEmpty
                              ? 'Einsatz Nr. $einsatzNr'
                              : 'Protokoll',
                        ...
                        onTap: id != null
                            ? () => Navigator.of(context).push(
                                  MaterialPageRoute(
                                    builder: (_) => EinsatzprotokollNfsDruckScreen(
      ...
      child: Column(
        ...
        children: [
          ...
          SegmentedButton<_FilterMode>(
            segments: const [
              ButtonSegment(
                  value: _FilterMode.jahr,
                  label: Text('Nach Jahr'),
                  icon: Icon(Icons.calendar_month, size: 18)),
              ButtonSegment(
                  value: _FilterMode.datum,
                  label: Text('Nach Datum'),
                ...
                child: _filterMode == _FilterMode.jahr
                    ? DropdownButtonFormField<int>(
                        ...
                        onChanged: (v) => setState(
                            () => _filterYear = v ?? DateTime.now().year),
                      )
                    : OutlinedButton.icon(
                        onPressed: _pickDate,
