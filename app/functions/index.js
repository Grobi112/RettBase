...
// Projekt explizit fÃÂÃÂÃÂÃÂ¼r Auth-Konsistenz (rett-fe0fa = Flutter-App + alle Module)
admin.initializeApp({
...
/** Rate-Limit fÃÂÃÂÃÂÃÂ¼r kundeExists und resolveLoginInfo: pro Client maximal 5 Aufrufe/Minute (Schutz vor Enumerations-Angriffen). */
...
function _checkKundeExistsRateLimit(context) {
  ...
  if (entry.count > KUNDE_EXISTS_MAX_PER_MINUTE) {
    throw new functions.https.HttpsError("resource-exhausted", "Zu viele Anfragen. Bitte spÃÂÃÂÃÂÃÂ¤ter erneut versuchen.");
  }
}

/** Basis-URL der Web-App fÃÂÃÂÃÂÃÂ¼r Push-Klick-Links (zentral gehostet, z.B. app.rettbase.de). */
const WEB_APP_BASE_URL = "https://app.rettbase.de";

/** Push-Benachrichtigungen (und Badge bei geschlossener App ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ SW setzt Badge beim Push-Empfang). */
const PUSH_ENABLED = true;

/** Tauscht Auth gegen Custom-Token fÃÂÃÂÃÂÃÂ¼r iframe-Auth-Bridge.
 *  Nutzt context.auth (automatisch vom Callable-Client) ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ robuster als manuelles idToken.
 *  Fallback: idToken aus data (fÃÂÃÂÃÂÃÂ¼r Clients, die ihn explizit senden).
 */
  ...
  if (!uid) {
    ...
    if (idToken && typeof idToken === "string") {
      try {
        ...
        throw new functions.https.HttpsError("unauthenticated", "Token ungÃÂÃÂÃÂÃÂ¼ltig oder abgelaufen");
      }
    }
  }
  if (!uid) {
...
/** PrÃÂÃÂÃÂÃÂ¼ft ob der Aufrufer Admin/Superadmin/LeiterSSD ist (fÃÂÃÂÃÂÃÂ¼r createAuthUser, updateMitarbeiterPassword). */
async function _requireAdminRole(context, companyId) {
  ...
  // Admin-Superadmins (users/mitarbeiter in admin mit role superadmin) dÃÂÃÂÃÂÃÂ¼rfen in allen Firmen Admin-Aktionen ausfÃÂÃÂÃÂÃÂ¼hren
  if (companyId !== "admin") {
  ...
  throw new functions.https.HttpsError("permission-denied", "Nur Admin, Superadmin oder LeiterSSD kÃÂÃÂÃÂÃÂ¶nnen diese Aktion ausfÃÂÃÂÃÂÃÂ¼hren.");
}

/** Erstellt einen Firebase Auth Nutzer (Admin-Funktion).
 *  Wird von der Mitgliederverwaltung aufgerufen ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Admin bleibt eingeloggt.
 */
exports.createAuthUser = functions.region("europe-west1").https.onCall(async (data, context) => {
  ...
  try {
    ...
    if (code === "auth/email-already-in-use" || code === "auth/email-already-exists") {
      throw new functions.https.HttpsError("already-exists", "E-Mail bereits registriert. Nutzen Sie ÃÂÃÂ¢ÃÂÃÂÃÂÃÂPasswort setzenÃÂÃÂ¢ÃÂÃÂÃÂÃÂ bei bestehendem Mitglied.");
    }
...
exports.updateMitarbeiterPassword = functions.region("europe-west1").https.onCall(async (data, context) => {
  ...
  if (!isInCompany) {
    throw new functions.https.HttpsError("permission-denied", "Nutzer gehÃÂÃÂÃÂÃÂ¶rt nicht zu dieser Firma ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Passwort-ÃÂÃÂÃÂÃÂnderung nicht erlaubt.");
  }
  try {
 ...
 *  (mitarbeiter war allow read: if true ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ DSGVO/Sicherheitsrisiko). */
exports.resolveLoginInfo = functions.region("europe-west1").https.onCall(async (data, context) => {
  ...
  // ÃÂÃÂÃÂÃÂ¤ndert die Auth-Identity nicht ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Login muss immer mit pseudoEmail erfolgen
  if (docPseudo) {
    email = docPseudo;
...
/** PrÃÂÃÂÃÂÃÂ¼ft ob eine Kunden-ID existiert (ohne Auth, fÃÂÃÂÃÂÃÂ¼r Eingabe beim Start).
...
/** PrÃÂÃÂÃÂÃÂ¼ft ob der Aufrufer Superadmin ist (fÃÂÃÂÃÂÃÂ¼r loadKunden, Kundenverwaltung). */
...
/** LÃÂÃÂÃÂÃÂ¤dt alle Kunden (Firmen) ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ nur fÃÂÃÂÃÂÃÂ¼r Superadmin. Projekt: rett-fe0fa, Collection: kunden. */
...
/** Schreibt Mitarbeiter-Dokument (umgeht Firestore-Regeln fÃÂÃÂÃÂÃÂ¼r Web-App). Nur Admin/Superadmin/LeiterSSD. */
...
/** Stellt sicher, dass users-Dokument existiert (fÃÂÃÂÃÂÃÂ¼r Firestore-Zugriffsregeln nach Login).
 *  PrÃÂÃÂÃÂÃÂ¼ft, ob Aufrufer in mitarbeiter der Firma ist; erstellt users-Doc falls nÃÂÃÂÃÂÃÂ¶tig.
 *  LÃÂÃÂÃÂÃÂ¶st kundenId ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ docId auf (z.B. kkg-luenen ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ keg-luenen). */
...
/** Setzt Custom Claims fÃÂÃÂÃÂÃÂ¼r Storage-Regeln (companyId / superadmin). Nur bei ÃÂÃÂÃÂÃÂnderung, um Token-Invalidierung zu vermeiden. */
...
/** Schreibt users-Dokument (umgeht Firestore-Regeln fÃÂÃÂÃÂÃÂ¼r Web-App). Nur Admin/Superadmin/LeiterSSD. */
...
/** Callable: PrÃÂÃÂÃÂÃÂ¼ft ob FCM-Token fÃÂÃÂÃÂÃÂ¼r aktuellen Nutzer in Firestore ist (fÃÂÃÂÃÂÃÂ¼r Debug/Status-Anzeige). */
...
/** Summiert ungelesene Nachrichten ÃÂÃÂÃÂÃÂ¼ber alle Chats eines Nutzers (inkl. neuer Nachricht). */
...
// FIX: Hilfsfunktion zum LÃÂÃÂ¶schen eines abgelaufenen FCM-Tokens
...
/** Firestore-Trigger: Bei neuer Chat-Nachricht ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Push an EmpfÃÂÃÂÃÂÃÂ¤nger senden. */
      ...
      for (const uid of recipients) {
        try {
          ...
          if (mutedIds.includes(chatIdNorm)) {
            console.log("onNewChatMessage: Chat " + chatIdNorm + " stumm fÃÂÃÂÃÂÃÂ¼r uid=" + uid + ", Push ÃÂÃÂÃÂÃÂ¼bersprungen");
            continue;
          }
          const token = await getFcmToken(companyId, uid);
          if (!token) {
            console.log("onNewChatMessage: Kein FCM-Token fÃÂÃÂÃÂÃÂ¼r uid=" + uid);
            continue;
          }
          ...
          if (code.includes("registration-token-not-registered") || code.includes("invalid-registration-token")) {
            ...
            try {
              await _deleteStaleToken(companyId, uid);
              console.info("onNewChatMessage: Abgelaufener FCM-Token fÃÂÃÂÃÂÃÂ¼r", uid, "gelÃÂÃÂÃÂÃÂ¶scht");
            } catch (_) {}
          } else {
        ...
        try {
          const prefsRef = admin.firestore()
            ...
            if (mutedIds.includes(String(chatId).trim())) {
              console.log(`onNewGroupChat: Gruppe ${chatId} stumm fÃÂÃÂ¼r uid=${uid}, Push ÃÂÃÂ¼bersprungen`);
              continue;
            }
            const token = await getFcmToken(companyId, uid);
            if (!token) { console.log(`onNewGroupChat: Kein FCM-Token fÃÂÃÂ¼r uid=${uid}`); continue; }
            const totalUnread = await getTotalUnreadForUser(companyId, uid, chatId);
            await sendChatPush(
              token,
              "Zur Gruppe hinzugefÃÂÃÂ¼gt",
              `Du wurdest zu "${groupName}" hinzugefÃÂÃÂ¼gt.`,
              companyId,
              chatId,
              {},
              totalUnread
...
/** VollstÃÂÃÂÃÂÃÂ¤ndige LÃÂÃÂÃÂÃÂ¶schung eines Mitglieds (DSGVO). Entfernt alle personenbezogenen Daten:
...
/** Erstellt neues Mitarbeiter-Dokument (fÃÂÃÂÃÂÃÂ¼r Neuanlage). Nur Admin/Superadmin/LeiterSSD. */
...
/** Versendet externe E-Mails (z.B. aus E-Mail-Modul, Schnittstellenmeldung, ÃÂÃÂÃÂÃÂbergriffsmeldung).
...
exports.sendEmail = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (!emailRegex.test(toTrim)) {
      throw new functions.https.HttpsError("invalid-argument", "UngÃÂÃÂÃÂÃÂ¼ltige E-Mail-Adresse");
    }
    ...
    if (!user || !pass) {
      console.error("sendEmail: SMTP nicht konfiguriert. .env mit SMTP_USER anlegen, firebase functions:secrets:set SMTP_PASS ausfÃÂÃÂÃÂÃÂ¼hren.");
      throw new functions.https.HttpsError(
        "failed-precondition",
        "E-Mail-Versand ist nicht konfiguriert. Bitte .env mit SMTP_USER anlegen und firebase functions:secrets:set SMTP_PASS ausfÃÂÃÂÃÂÃÂ¼hren."
      );
    }
    ...
    if (messageId && String(messageId).trim()) {
      htmlBody += `<p style="margin-top:24px;font-size:11px;color:#888;">ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ<br>Gesendet ÃÂÃÂÃÂÃÂ¼ber RettBase. Nur Antworten auf diese E-Mail sind mÃÂÃÂÃÂÃÂ¶glich. Direktmails an mail@rettbase.de werden nicht bearbeitet.</p>`;
    }
...
/** Pollt IMAP-Inbox fÃÂÃÂÃÂÃÂ¼r eingehende Antworten auf externe E-Mails.
 ...
 *  LÃÂÃÂÃÂÃÂ¤uft jede Minute. IMAP: IMAP_HOST, IMAP_PORT, SMTP_USER, SMTP_PASS. */
...
exports.pollInboundEmail = functions
  ...
  .onRun(async () => {
    ...
    if (!user || !pass) {
      console.warn("pollInboundEmail: SMTP_USER/SMTP_PASS nicht gesetzt, ÃÂÃÂÃÂÃÂ¼berspringe.");
      return null;
    }
